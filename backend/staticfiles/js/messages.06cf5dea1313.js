...
function initChatPage() {
    ...
    // Получаем ID текущего пользователя из сессии (устанавливается при авторизации)
    // Для этого можно добавить data-атрибут в template или получить через API
    _currentUserId = parseInt(page.dataset.otherId); // временно, нужно получить свой ID
    ...
    _chatSocket.onmessage = (e) => {
        ...
        switch (data.type) {
            ...
            case 'chat_message':
                ...
                appendMessage(msgArea, data.message);
                scrollToBottom(msgArea);
                break;
